...
이미지는 크 두가지 영역으로 표현할 수 있다. 하나는 공간영역 다른하나는 변환영역/주파수 영역.

그중 주파수 영역에서는 공간 주파수라는 것을 사용하는데 이것에 대해 중점적으로 알아보자 !

공간영역은 픽셀 값으로 이미지르 표현/ 우리가 일반적으로 생각하는 이미지 표현 방식. 

반면 주파수 영역에서느 화소 값이 직접 표현되지 않고 변동계수로 표현된다. 

즉, 공간영역 상에서 화소 밝기의 변화율을 나타낸다고 공간 주파수라는 표현을 사용. 

고주파 / 저주파

공간 주파수는 밝기가 얼마나 빨리 변화하는가에 따라서 고주파 영역과 저주파 영역으로 분류한다. 

고주파 영역은 밝기가 빨리 변화하는 영역으로 주로 경계부분이나 객체의 모서리 부분, 저주파 영역은 밝기가 거의 변하지 않느 영역으로 주로 배경 부분이나 객체의 내부에 많이 있다.

주파수 처리를 통해 고주파와 저주파로 구분이 가능하다면 다양한 응용이 가능하다. 
ex) 고주파 영역의 성분을 제거하여 경계가 흐려진 영상을 만들거나 저주파 영역의 성분을 제거하여 경계나 모서리만 포함하는 영상인 에지 영상을 만들수 있다. 



step -> 1. 주파수 변환 (입력 영상을 주파수 영역으로 변환) 2. 해당 주파수 영역 처리(주파수를 원하는 데로 변환) 3. 주파수 역변환 (주파수를 공간 영역의 영상으로 변환) 



*******이산 푸리에 변환 ***********

이산 푸리에 변환이 쓰이는 이유는 무엇일까? -> 푸리에 급수와 변환의 가정은 시간 영역의 신호가 연속이어야 한다느 점인데, 컴퓨터 등과 같은 디지털 신호 처리 장치는 모든 자료를 연속적으 저장할 수 없다.

대신에, 디지털 장치에 입력되는 시간 함수도 이산적이고 변환된 주파수 함수도 이산적으 저장될 수 밖에 없기 때문에 이산 푸리에 변환이 사용된다. 

이렇게 시간영역과 주파수 영역 모두에서 신호 함수가 이산적이기 위해서는 시간 영역에서의 신호가 이산성을 가져야 할 뿐만 아니라 주기성도 가져야 한다. 그 이유는, 이산성과 주기성은 시간 영역과 주파수 영역에서 서로 대응하기 때문이다. 즉 시간 영역에서의 주기성을 가진 함수는 주파수 영역에서 이산성을 갖게 되고 그 반대도 마찬가지로 성립한다. 

그러나 현실에서 디지털 장치에서 이산 푸리에 변환을 적용하고자 하는 함수가 항상 주기적인 함수만 가지느 것이 아니라 비주기 함수이 경우도 많다. 
이런 경우에도 문제가 없는 것이 디지털 장치는 비주기적인 입력 신호 함수를 T 시간 입력 받아서 저장해 두고느 이것이 마치 t주기마다 반복되는 주기 함수인것처럼 취급하여 이산 푸리에 변환을 적용할 수 있으며 표본화 시간(sampling time) 만 충분히 작다면 아무런 문제가 없기때문이다. 또한, 주파수 영역의 신호 함수를 시간 영역으로 다시 복원을 하면 T의 주기를 갖는 주기 함수가 만들어지지만 T 한 주기만을 출력하면 원래의 신호가 복원되기 때문이다. 



참고로, '고속 푸리에 변환'(Fast Fourier Transform, FFT)은 이 이산 푸리에 변환의 계산 시간을 줄이기 위한 하나의 알고리즘으로서 N개의 표본 자료를 홀수 번호와 짝수 번호로 나누어 계산함으로써 계산 과정 중의 같은 연산 값들을 재사용함으로써 계산량을 O(N*N)에서 O(N*log N)으로 줄이며 또한 재귀적 함수 호출을 통해 알고리즘이 간단한 장점을 갖는다.
